// WebSQL polyfill using localStorage (Chrome removed openDatabase in 2022)
(function(w){
  if(w.openDatabase)return;
  function gs(){try{return JSON.parse(localStorage.getItem('ae_db')||'{}')}catch(e){return{}}}
  function ss(s){try{localStorage.setItem('ae_db',JSON.stringify(s))}catch(e){}}
  function T(s){this.s=s}
  T.prototype.executeSql=function(q,p,ok,er){
    var s=this.s,r={rows:{length:0,item:function(){return null}}};
    try{
      q=q.trim();
      if(/^CREATE TABLE/i.test(q)){var t=(q.match(/EXISTS (\w+)/i)||[])[1];if(t&&!s[t])s[t]=[];return ok&&ok(this,r);}
      if(/^DROP TABLE/i.test(q)){var t=(q.match(/EXISTS (\w+)/i)||[])[1];if(t)delete s[t];ss(s);return ok&&ok(this,r);}
      if(/^INSERT/i.test(q)){
        var t=q.match(/INTO (\w+)/i)[1],
            c=q.match(/\(([^)]+)\)\s*values/i)[1].split(',').map(function(x){return x.trim();}),
            row={};
        c.forEach(function(k,i){row[k]=p[i];});
        if(!s[t])s[t]=[];s[t].push(row);ss(s);return ok&&ok(this,r);
      }
      if(/^DELETE/i.test(q)){
        var t=q.match(/FROM (\w+)/i)[1],wh=q.match(/WHERE (\w+)\s*=/i);
        if(s[t])s[t]=wh?s[t].filter(function(x){return x[wh[1]]!==p[0];}):[];
        ss(s);return ok&&ok(this,r);
      }
      if(/^SELECT/i.test(q)){
  ...
  D.prototype.transaction=D.prototype.readTransaction=function(cb,er,ok){
    var tx=new T(this.s);
    try{cb(tx);ss(tx.s);ok&&ok();}catch(e){er&&er(e);}
  };
